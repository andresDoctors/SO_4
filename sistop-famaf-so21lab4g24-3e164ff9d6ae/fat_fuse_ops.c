...
static void fat_fuse_log_activity(char *operation_type, fat_file target_file)
{
    ...
    fat_tree_node spy_node = fat_tree_node_search(get_fat_volume()->file_tree, "/fs.log");
    fat_file spy_file = fat_tree_get_file(spy_node);
    fat_file_pwrite(spy_file, buf, strlen(buf), spy_file->dentry->file_size, fat_tree_get_parent(spy_node));
...
static int fat_fuse_create_spy() {
    fat_tree_node spy_node = fat_tree_node_search(get_fat_volume()->file_tree, "/fs.log");

    if (spy_node == NULL) {
        if (fat_fuse_mknod("/fs.log", 0u, 0ul) < 0) {
            DEBUG("ERROR: fat_fuse_ops.c -> fat_fuse_create_spy() -> fat_fuse_mknod()");
            return -1;
        }
        spy_node = fat_tree_node_search(get_fat_volume()->file_tree, "/fs.log");


        fat_file file = fat_tree_get_file(spy_node);
        memmove(file->dentry->base_name + 1, file->dentry->base_name, (8 - 1) * sizeof(u8));
        fat_file_update(file, fat_tree_get_parent(spy_node), 0xE5, 1); 
    }
...
static int fat_fuse_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    if (fat_file_cmp_path(file, "/fs.log")) {
...
int fat_fuse_unlink(const char* path) {
    ...
    fat_tree_node file_node = fat_tree_node_search(get_fat_volume()->file_tree, path);
    fat_file file = fat_tree_get_file(file_node);
    u32 cluster, cluster_aux;

    cluster = file->start_cluster;
    while (!fat_table_is_EOC(file->table, cluster)) {
        cluster_aux = fat_table_get_next_cluster(file->table, cluster);
    ...
    fat_file_update(file, fat_tree_get_parent(file_node), FAT_FILENAME_DELETED_CHAR, 0);
    fat_tree_delete(get_fat_volume()->file_tree, path);
