...
static bool ignore_dentry(const fat_dir_entry disk_dentry) {
    ...
    if (disk_dentry->base_name[0] == 0xE5) {
        if (strcmp( (char*) &(disk_dentry->base_name[1]), "fs") == 0) {
            return false;
        }
    }
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    while (bytes_remaining > 0) {
        ...
        if (bytes_remaining > 0) {
            ...
            if (fat_table_is_EOC(file->table, cluster_aux)) {
                cluster_aux = fat_table_get_next_free_cluster(file->table);
                fat_table_set_next_cluster(file->table, cluster, cluster_aux);
                fat_table_set_next_cluster(file->table, cluster_aux, FAT_CLUSTER_END_OF_CHAIN);
            }
...
void fat_file_update(fat_file file, fat_file parent, int new_basename,
                     bool b) {
    file->dentry->base_name[0] = new_basename;
